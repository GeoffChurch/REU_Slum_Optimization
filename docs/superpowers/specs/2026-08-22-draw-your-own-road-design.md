...
copy would let the browser run different code from CI while both looked healthy. Three consumers,
two builds:

* `web/test/pyodide-parity.test.ts` loads it from `dist/` through Node's filesystem, after
  `web/scripts/test.sh` has invoked the `wheel` task.
* `tests/test_gen_site_pages.py` renders the `DRAWROAD` producer, which raises without it — so
  `test-py` depends on the same `wheel` task.
* `deploy-site.yml` builds it with its own pip, and `gen_site_pages.py` copies it into
  `docs/assets/`; the mount point carries the resulting path as `data-wheel`, rewritten by
  `_write_page` exactly as `data-bundle` is.
...
1. **A package that left the distribution.** **Measured at seven pinned paths** (`pyodide-lock.json`
   at each): `geopandas` and `pyproj` are present in 0.26.4, 0.27.0 and 0.27.7, **absent at 0.28.0,
   0.29.0 and 0.29.1**, and present again in 0.29.2. `shapely` is present in every one of them. So
   the disappearance is **two packages, absent at three of the seven paths measured**, not three
   packages across two releases — and it is still the reason to pin, because the two that vanished
   are load-bearing here. Whether a release between 0.28.0 and 0.29.1 that was not fetched also
   lacks them is unmeasured; the seven above are what was checked.
   (This sentence has now been wrong three times, each time by claiming a span wider than the
   fetches behind it: "removed in 0.27", then "absent in 0.28.0" when 0.29.0/0.29.1 had not been
   looked at, then "across four releases" when three paths showed the absence. It is stated as a
   count of measured paths for that reason.)
...
Only the first of these lands on the committed PNG. That is what the fallback exists for, and it
covers exactly the case where the widget never runs; once the widget has drawn, it has removed that
image, so every later failure has to land in the widget's own surface instead:
...
Two categories are deliberately absent from the tables. This spec and its plan
(`docs/superpowers/specs/2026-08-22-draw-your-own-road-design.md`,
`docs/superpowers/plans/2026-08-22-draw-your-own-road.md`) are the documents describing the work
rather than files it produces. And `web/src/py/solve-source.generated.ts` and
`web/src/render/graph-encoding.generated.ts` are build outputs of the two `web/scripts/gen-*.mjs` above:
gitignored, regenerated by `npm run check` and `npm test` before anything reads them, and never
hand-edited.
