...
- [ ] **Step 2: Implement `run_dual_target`** — mirror the structure of the current `run_two_lens` (reblock each method once — over-provisioned config already applied by the caller; the region block scores every method), but replace the two-lens body with:
  ...
  3b. Render ONE `before.jpg` per example: the region's status-quo access-depth heatmap with NO roads, via `render_before(block, kc.fields["access_before"], vmax=vmax, frame=frame_bbox(block.parcels))` using any method's `kc` (`access_before` is method-independent) and the same shared `vmax`. This gives the multiblock examples the before/after pair they currently lack (method-comparison already had one — this unifies it, so `gen_method_comparison.py` drops its own before-render and relies on `run_dual_target`).
