 ...
 * per-building contribution `cᵢ` that shades each building. `c` is passed in rather than recomputed
 ...
 *   2. the road corridor, translucent, UNDER everything above it -- a corridor drawn over the
 *      buildings would tint the shading it exists to be read against (`_draw_corridor`, zorder 2);
 *   3. the block boundary and the existing streets (`_draw_boundary_and_streets`);
 *   4. EVERY building as its outline at the block's tier: the ones the corridor misses as a thin
 *      outline (zorder 5), the ones it takes a share of filled at `alpha = cᵢ` (zorder 6). Drawing
 *      the zero-cost buildings too is the whole point -- without them a reader cannot see that a
 *      road threaded a GAP, only that some homes went red;
 *   5. the drag handles, LAST, so a handle is never buried under a building it happens to sit on.
 *
...
export function drawField(ctx: CanvasRenderingContext2D, b: FieldBundle, f: FieldFrame,
  ...
  // (4) The buildings. Two passes in `render_field`'s own zorder order: the untouched ones as
  // outlines first, the ones the corridor takes a share of filled at their own alpha on top. Every
  // ring of a building goes in ONE path, so the default nonzero fill leaves a hole unfilled: the
  // bake orients holes opposite to their exterior.
  const outlinePath = (i: number): void => {
    ctx.beginPath();
    for (const poly of b.buildings[i]!) {
  ...
  ctx.strokeStyle = e.building_color;
  ctx.lineWidth = e.outline_lw;
  for (let i = 0; i < b.buildings.length; i++) {
    // `f.c` has one entry per building: `contributions` builds it from these same outlines, one
    // `Outline` each, in order.
    if (f.c[i]! > 0) continue;
    outlinePath(i);
    ctx.stroke();
  }
  ctx.fillStyle = e.building_color;
  for (let i = 0; i < b.buildings.length; i++) {
    const ci = f.c[i]!;
    if (ci <= 0) continue;
    // Opacity IS the cost: a building the corridor barely clips is nearly transparent, one it
    // swallows solid. Same encoding as `render_field`'s per-building facecolor alpha.
    ctx.globalAlpha = ci;
    outlinePath(i);
    ctx.fill();
  }
