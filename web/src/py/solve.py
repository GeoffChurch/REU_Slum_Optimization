...
`reblock.buildings`/`reblock.contracts`/`reblock.derive.access`/`reblock.permeability` themselves
-- are exactly what `reblock.permeability` already pulls in transitively (via `reblock.contracts`,
which it imports `Block` from, and which itself imports `reblock.buildings`; and
`reblock.derive.access`, which it imports `ParcelAdjacency` from). Nothing here adds a package to
the browser's install.
...
It calls `solve_egress`, NOT `permeability`: `permeability` also solves the no-roads baseline
(once per `EgressContext`, on its first call), and that baseline is road-invariant (design §1.6)
and already baked into `examples/authoring/block.json` as `baseline.p0`. One solve per edit is
what makes re-solve-while-dragging affordable. MEASURED (Task 2 fault injection 2): the two agree
bit for bit on both reference roads, so this choice is about the COST of a second solve, not about
the two paths disagreeing numerically.
...
class BaselineDict(TypedDict):
    """`EgressContext.baseline`'s own answer -- `web/src/authoring.d.ts`'s `baseline`."""
    p0: float
...
def adjacency_from_bundle(bundle: AuthoringBundle, block: Block) -> ParcelAdjacency:
    ...
    Paired with `block` here, the block rebuilt from the same bundle, so the two cannot come from
    different ones: `ParcelAdjacency` refuses neighbour sets whose count is not the block's parcel
    count.
    """
    ...
    for i, j in zip(bundle["edges"]["rows"], bundle["edges"]["cols"], strict=True):
        adj[i].add(j)
        adj[j].add(i)
    return ParcelAdjacency(block, STREET_TOL, adj)
...
def solve(ctx: EgressContext, road: list[list[float]], p0: float) -> PyResult:
    ...
    `p0` (the bundle's baked no-roads baseline) rather than `ctx.baseline`, which would cost the
    first edit a second solve.

    `ctx` is built once at boot by `context_from_bundle` -- see `adjacency_from_bundle` for why its
    baked adjacency is the difference between a widget that works on a settlement block and one
    that kills the interpreter.
    ...
    roads = gpd.GeoDataFrame(
        {"geometry": [line], WIDTH_COL: [ctx.params.min_road_width_m]},
        geometry="geometry", crs=ctx.block.crs)
