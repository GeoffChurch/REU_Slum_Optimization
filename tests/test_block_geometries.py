"""KblockSource.block_geometries: building_count_raw is exposed alongside block_id + geometry
...
def test_block_geometries_includes_building_count() -> None:
    ...
    assert int(row.building_count_raw) == 53           # matches the parquet
...
def test_block_geometries_building_count_survives_bbox_window() -> None:
    ...
    assert "building_count_raw" in windowed.columns
