...
def test_it_scores_the_UNION_not_the_candidate_block():
    ...
    got = ShapeStandardizingRegionBuilder(max_buildings=40,
                                          objective=Squareness()).build(geoms, [["2_2"]])[0]
...
def test_it_beats_dense_cluster_on_outline_for_the_same_budget():
    ...
    geoms = _grid(6)
    seed, budget = [["3_3"]], 60
    shaped = ShapeStandardizingRegionBuilder(max_buildings=budget,
                                             objective=Squareness()).build(geoms, seed)[0]
...
def test_the_seed_is_always_kept_even_alone_over_budget():
    geoms = _grid(3)
    got = ShapeStandardizingRegionBuilder(max_buildings=1,
                                          objective=Squareness()).build(geoms, [["1_1"]])[0]
    assert got == ["1_1"]


def test_growth_is_contiguous_and_deterministic():
    geoms = _grid(5)
    runs = [ShapeStandardizingRegionBuilder(max_buildings=50,
                                            objective=Squareness()).build(geoms, [["0_0"]])[0]
            for _ in range(3)]
...
def test_it_works_without_building_counts():
    ...
    got = ShapeStandardizingRegionBuilder(max_buildings=4,
                                          objective=Squareness()).build(geoms, [["1_1"]])[0]
...
def test_an_unknown_seed_id_fails_with_a_named_error():
    ...
    with pytest.raises(ValueError, match="nope"):
        ShapeStandardizingRegionBuilder(objective=Squareness(),
                                        max_buildings=150).build(geoms, [["nope"]])
