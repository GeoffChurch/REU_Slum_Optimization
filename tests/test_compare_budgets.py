...
def test_run_permeability_lenses_writes_tables_and_renders(tmp_path: Path) -> None:
    # End-to-end glue smoke test on a tiny 2-block region with a real reblocker (ClearanceReblocker,
    # default depth_target=2 -- brings the block's max access depth down to 2). Asserts the
    # frontier + the two lens tables + a before/after render per lens per coloring + the per-method
    # GIF are all written from the SAME reblock (no second propose), and that no retired two-lens/
