"""Bake examples/authoring/ -- the block the draw-your-own-road widget reconstructs in the browser.
...
serve bundles that get drawn -- and on this block cm rounding moves permeability from 0.9508990541
to 0.9508519775, a difference of 4.71e-05 (design §1.4). `web/test/pyodide-parity.test.ts` compares
the browser's answer against `reference` below to decide whether the WASM runtime agrees with
CPython; a tolerance wide enough to absorb quantisation would no longer be measuring that.

The block is ZAF.9.3.1_1_40972, the same one PermGraph, Frontier, DisplacementField, RegionGrow and
ScreenMap pin.
...
from reblock.mesh import footpath_mesh, parcel_radii
...
/** One fixed road and CPython's own permeability for it, baked so `pyodide-parity.test.ts`
 * needs no Python process at test time -- the same device `field.json` and `hood.json` use. */
...
export interface AuthoringBlock {
  ...
  crs_epsg: number;
  /** FULL float64, NOT the cm-rounded form the render bundles ship (design §1.4): quantising
   * moves permeability by 4.71e-05 on this block, which is more than §5's guard can tolerate.
...
def _assert_round_trip(bundle: AuthoringBundle, params: PermeabilityParams) -> None:
    ...
    of any size means the JSON has lost something, which is the thing worth hearing about.
    """
...
def main() -> None:
    ...
    # One mesh and one baseline solve. `footpath_mesh` takes no roads and `solve_egress` returns
    # the assembly it solved, so this is where every road-invariant column below comes from.
    mesh = footpath_mesh(block, params, radii=parcel_radii(block, params))
    baseline = solve_egress(block, None, params)
    ...
    log.info("round trip: %d reference roads reproduced exactly from the bundle", len(reference))
