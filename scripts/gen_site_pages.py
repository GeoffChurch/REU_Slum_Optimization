"""Machine-generated MkDocs site pages. PURE dir-reader in the spirit of
scripts/gen_example_readme.py: every number, table, and image on the generated pages is read from
...
never-stale) site. Prose blurbs are the only handwritten content.

Usage:  python3 scripts/gen_site_pages.py
Emits:  docs/methods/<method>.md, docs/benchmark.md, and copies the referenced images into
...
GENERATED_NOTE = ("<!-- Generated by scripts/gen_site_pages.py from run artifacts on disk. "
                  "Do not edit numbers by hand - regenerate instead. -->\n")
...
class M:
    def __init__(self, slug: str, title: str, blurb: str, *, mc_key: str | None = None,
                 ...
                 status: str | None = None, extra: str = "") -> None:
        ...
        self.conf, self.status, self.extra = conf, status, extra
...
def _mc_section(m: M) -> list[str]:
    ...
    parts.append("From the [method-comparison example](https://github.com/jmendoza167/"
                 ...
                 "reach every parcel.\n")
    if before and after:
        parts.append(_img_row([("before", f"../{before}"), (f"after `{m.mc_key}`", f"../{after}")]))
    if summary:
...
def _mb_section(m: M) -> list[str]:
    ...
    imgs = [(lbl, f"../{u}") for lbl, u in (("matched road budget", matched),
...
def _k_section(m: M) -> list[str]:
    ...
    if pair:
        parts.append("\n" + _img_row([("before", f"../{pair[0]}"), ("after", f"../{pair[1]}")]))
    return parts


def gen_method_page(m: M) -> str:
    parts = [GENERATED_NOTE, f"# {m.title}\n"]
    if m.status:
    ...
    parts += _mc_section(m) if m.mc_key else []
    parts += _mb_section(m) if m.mb_key else []
    parts += _k_section(m)
    if m.slug == "peel" and len(parts) <= 4:
        parts.append("*No evaluation artifacts for this method were found on disk.*\n")
    return "\n".join(parts) + "\n"


def gen_benchmark_page() -> str:
    import json
    parts = [GENERATED_NOTE, "# Benchmarks\n"]
...
def main() -> None:
    ...
    for m in METHODS:
        path = DOCS / "methods" / f"{m.slug}.md"
        path.write_text(gen_method_page(m))
        print(f"wrote {path.relative_to(ROOT)}")
    bench = DOCS / "benchmark.md"
    bench.write_text(gen_benchmark_page())
